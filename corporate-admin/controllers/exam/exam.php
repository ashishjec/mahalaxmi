...
$que = $_GET['que'];
$exam = $_GET['exam'];
$course_content_id  = $_GET['section'];
...
$select_table2= "SELECT * FROM `course_contents`  WHERE status = 'Active' and id = '$course_content_id' and crs_cnt_course_id = '$table_id' order by id desc";
...
//echo "=========== $crs_time";
$time_used = 0;
$time_used  = "SELECT SUM(TIME(st_timer_time_diff)) as time_used FROM student_timer where st_timer_student = '$master_user_id' and st_timer_exam = '$exam' ";
...
if($sql7->rowCount() > 0){
	...
	{
		$time_used_prev = $row7->time_used;
		//echo "$time_used =============";
		$time_used = substr("$time_used_prev",3,2);
...
if(isset($_POST['submitNext'])){
	...
	$temp_que_no = $_POST['temp_que_no'];
	$temp_option = $_POST['option'];
	$temp_question = $_POST['temp_question'];
	$temp_option_1 = $_POST['option_1'];
	$temp_option_2 = $_POST['option_2'];
	$temp_option_3 = $_POST['option_3'];
	$temp_option_4 = $_POST['option_4'];
	$temp_option_5 = $_POST['option_5'];
	$temp_correct_marks = $_POST['temp_correct_marks'];
	$temp_incorrect_marks = $_POST['temp_incorrect_marks'];
	$temp_correct_answer = $_POST['temp_correct_answer'];

	
  
  if ($temp_option == $temp_correct_answer & $que > 0) {
    	$start_exam = "INSERT `student_exam_responses` SET
					   ...
					   std_exm_question = '$temp_question',
					   std_exm_option_1 = '$temp_option_1',
					   std_exm_option_2 = '$temp_option_2',
					   std_exm_option_3 = '$temp_option_3',
					   std_exm_option_4 = '$temp_option_4',
					   std_exm_option_5 = '$temp_option_5',
					   std_exm_correct_wrong = 'Correct',
					   std_exm_correct_answer = '$temp_correct_answer',
					   std_exm_rsp_answer = '$temp_option',
					   std_exm_rsp_marks = '$temp_correct_marks',
					  status   = 'Active'"; 
    }elseif ($temp_option == $temp_correct_answer & $que == 0) {
    	$start_exam = "INSERT `student_exam_responses` SET
					   std_exm_rsp_user_id = '$master_user_id',
					   std_exm_rsp_exam_id = '$exam',
					   std_exm_sl_no = '$temp_que_no',
					   std_exm_section_no = '$course_content_id',
					   std_exm_question_no = 0,
					   std_exm_question = '$temp_question',
					   std_exm_option_1 = '$temp_option_1',
					   std_exm_option_2 = '$temp_option_2',
					   std_exm_option_3 = '$temp_option_3',
			    	...
			    	$start_exam = "INSERT `student_exam_responses` SET
					   ...
					   std_exm_option_4 = '$temp_option_4',
					   std_exm_option_5 = '$temp_option_5',
					   std_exm_correct_wrong = 'InCorrect',
					   std_exm_correct_answer = '$temp_correct_answer',
					   std_exm_rsp_answer = '$temp_option',
					   std_exm_rsp_marks = '-$temp_incorrect_marks',
					  status   = 'Active'"; 
			}elseif ($temp_option != $temp_correct_answer & $que == 0) {
			    	$start_exam = "INSERT `student_exam_responses` SET
					   std_exm_rsp_user_id = '$master_user_id',
...
$delete_statement = "UPDATE `student_exam_responses` SET
      status   = 'Inactive'
      where std_exm_rsp_user_id = '$master_user_id' and
					   std_exm_rsp_exam_id = '$exam' and
					   std_exm_sl_no = '$temp_que_no' and
					   std_exm_section_no = '$course_content_id' and
					   std_exm_question_no = ($que + 1) and
					   id < '$lastId'";   
...
$start_exam1 = "UPDATE `student_exams` SET
   ...
   WHERE
   std_exm_exam_id = '$exam' AND std_exm_user_id = '$master_user_id'";   
...
$myid = $dbconn->lastInsertId();
  ...
  header("Location: exam.php?id=$table_id&question=$temp_que_no&que=$temp_que&exam=$exam&section=$course_content_id&00=$eeeeid");
...
$select_table3B= "SELECT * FROM `course_contents`  WHERE status = 'Active' and id > '$course_content_id' and crs_cnt_course_id = '$table_id' order by id desc";
 $sql3B=$dbconn->prepare($select_table3B);
 $sql3B->execute();
 $wlvd3B=$sql3B->fetchAll(PDO::FETCH_OBJ);
 foreach($wlvd3B as $rows3B);
 $crs_cnt_name_3B  = $rows3B->crs_cnt_name;
 $course_content_id_3B  = $rows3B->id;
 $crs_time_3B  = $rows3B->crs_time;

  


if ($course_content_id_3B == '') {
	...
	?>
		<div class="row" style="padding-top: 100px; ">
			...
			<div class="col-md-6" style="text-align: right;">
				<a href="exam.php?id=<?php echo $table_id?>&question=0&exam=<?php echo $exam?>&section=<?php

$select_table22= "SELECT * FROM `course_contents`  WHERE status = 'Active'  and crs_cnt_course_id = '$table_id' order by id desc";
 $sql22=$dbconn->prepare($select_table22);
 $sql22->execute();
 $wlvd22=$sql22->fetchAll(PDO::FETCH_OBJ);
 foreach($wlvd22 as $rows22);
 $crs_cnt_name3  = $rows22->crs_cnt_name;
 $course_content_id3  = $rows22->id;
 $crs_time3  = $rows22->crs_time;
echo "$course_content_id3";



				?>" class="btn btn-warning btn-lg btn-block">Start Over Again</a>
			</div>
			...
			$sql_insert->execute();
			 ...
			 header("Location: result.php?exam=$table_id");
			}
	...
	?>
	<div class="row" style="padding-top: 100px; ">
			<div class="col-md-12" style="text-align: center; font-size: 30px; ">Section Completed
			</div>
			<div class="col-md-12" style="text-align: center; font-size: 20px; padding-bottom: 40px">Click On Start Next Button to Continue to the next section.
			</div>

			 
			<div class="col-md-6" style="text-align: right;">
				<a href="exam.php?id=<?php echo $table_id?>&question=0&exam=<?php echo $exam?>&section=<?php echo $course_content_id_3B?>" class="btn btn-warning btn-lg btn-block">Next Section</a>
			</div>
			<div class="col-md-6" style="text-align: left;">
...
if ($que == '') {
	...
	?>
<a href="exam.php?id=<?php echo $table_id?>&exam=<?php echo $exam?>&question=<?php echo ($question_no - 2)?>&section=<?php echo $_GET['section']?>&que=<?php 

$select_table4= "SELECT * FROM `questions`  WHERE status = 'Active' and ques_section_id = '$course_content_id' and id < '$que' order by id ASC";
	...
	<?php
			...
			<div class="col-md-4" style="text-align: center;">
				...
				--->
       				...
       				<input type="hidden" name="temp_question" value="<?php echo $rows3->ques_question?>">
       				<input type="hidden" name="option_1" value="<?php echo $rows3->ques_option_1;?>">
       				<input type="hidden" name="option_2" value="<?php echo $rows3->ques_option_2;?>">
       				<input type="hidden" name="option_3" value="<?php echo $rows3->ques_option_3;?>">
       				<input type="hidden" name="option_4" value="<?php echo $rows3->ques_option_4;?>">
       				<input type="hidden" name="option_5" value="<?php echo $rows3->ques_option_5;?>">
       				...
       				<input type="hidden" name="temp_correct_answer" value="<?php echo $rows3->ques_correct_answer?>">
       				<input type="hidden" name="temp_correct_marks" value="<?php echo $rows3->ques_mark?>">
       				<input type="hidden" name="temp_incorrect_marks" value="<?php echo $rows3->ques_negative_mark?>">
...
//Update $que no where $que = 1


$select_table11= "SELECT * FROM `student_exam_responses`  WHERE   std_exm_rsp_user_id = '$master_user_id' and std_exm_rsp_exam_id = '$exam' and std_exm_section_no = '$course_content_id'";  
$sql11=$dbconn->prepare($select_table11);
$sql11->execute();
...
if($sql11->rowCount() > 0){
 ...
 $que_1_id  = $rows11->id;
 $que_1_std_exm_sl_no  = $rows11->std_exm_sl_no;
 $que_1_std_exm_question_no  = $rows11->std_exm_question_no;
 $que_temporary = $_GET['que'];

 


if($que_1_std_exm_question_no > '0'){
	$que_statement = "UPDATE `student_exam_responses` SET
      std_exm_question_no   = '$que_temporary'
      WHERE

      std_exm_rsp_user_id = '$master_user_id' and 
      std_exm_rsp_exam_id = '$exam' and 
      std_exm_sl_no = '1' and 
       std_exm_question_no IN ('0', '') and 
      std_exm_section_no = '$course_content_id'
      
       ";   
  
  
$sql_insert = $dbconn->prepare($que_statement);
$sql_insert->execute();
